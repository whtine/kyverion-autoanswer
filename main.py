...
# --- ЛОГИКА ИИ (Копия твоего JS подхода) ---

async def ask_gemini(user_text, zone_context, remaining_msgs):
    if not GEMINI_KEY:
        return "Привет! Я сейчас занят, отвечу позже.", 1
        
    # Твой рабочий эндпоинт из JS
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash-preview:generateContent?key={GEMINI_KEY}"
    headers = {'Content-Type': 'application/json'}
    
    # Промпт на русском/украинском
    prompt = (
        f"Ты ИИ-ассистент владельца аккаунта. Владелец сейчас: {zone_context}. "
        f"Ответь очень коротко и вежливо на языке пользователя. "
        f"В конце добавь: (ИИ. Осталось: {remaining_msgs}). "
        f"Оцени важность сообщения от 1 до 3 и в самом конце припиши [P:X]."
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if resp.status_code != 200:
                ...
                return "Дякую за повідомлення! Я відповім найближчим часом.", 1
            ...
            return "Дякую! Скоро буду на зв'язку.", 1
            
        except Exception as e:
            logger.error(f"AI critical Error: {e}")
            return "Наразі система обробки запитів недоступна. Я зв'яжуся з вами позже.", 1
...
async def business_handler(message: types.Message):
    ...
    if u and u['is_active']:
        ...
        # Контекст Чехия (UTC+2)
        h = (datetime.utcnow().hour + 2) % 24
        zone = "обычное время"
        if 8 <= h < 13: zone = "на учебе (отвечает раз в час)"
        elif h >= 23 or h < 8: zone = "спит (ответит утром)"

        ai_reply, priority = await ask_gemini(message.text, zone, 10 - count)
...
async def delayed_ai_reply(message, text, delay, priority, count, conn):
    try:
        ...
        if priority >= 2:
            ...
            await bot.send_message(MY_ID, f"{tag} от {message.from_user.full_name}:\n{message.text}\n\nИИ ответил: {text}")
...
async def start_cmd(message: types.Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🗑 Сбросить лимиты", callback_data="clear_limits")]
    ])
    
    await message.answer(f"🤖 **ИИ-Ассистент: Gemini 3 Flash**\n\nСтатус: {status}\nЧехия (UTC+2)\nЛимит: 10 сообщ/чел.", reply_markup=kb)
...
async def init_db():
    conn = await get_conn()
    await conn.execute('CREATE TABLE IF NOT EXISTS users (user_id BIGINT PRIMARY KEY, is_active BOOLEAN DEFAULT TRUE, delay_sec INTEGER DEFAULT 30)')
